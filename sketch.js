...
function preload(){
  trex_running = loadAnimation("j.png","k.png");
  trex_collided = loadAnimation("c3.png");
...
function setup() {
 ...
 console.log(message)
  ...
  ground = createSprite(width/2,300,400,20);
  ground.addImage("ground",groundImage);
  ground.x = ground.width /2;
  
  gameOver = createSprite(width/2,height/2-130);
  ...
  invisibleGround = createSprite(width/2,335,width,20);
...
function draw() {
  ...
  if(gameState === PLAY){
    ...
    if(touches.lenght>0 || keyDown("space")&& trex.y >= 210) {
        trex.velocityY = -12;
        jumpSound.play();
        touches = [];
    }
